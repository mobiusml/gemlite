# pip install bitblas;
# pip install https://github.com/IST-DASLab/marlin.git;
# pip install vllm;
...
# pip install hqq;
# OMP_NUM_THREADS=16 CUDA_VISIBLE_DEVICES=0 TRITON_PRINT_AUTOTUNING=1 ipython3 triton_248_kernel_gemm.py
...
#############################################################################################################################################
# #Merge zero/scale in uint16
# zero = torch.tensor([[5.0078],
#         [8.0000],
#         [7.0547],
#         [8.0469],
#         [7.1328],
#         [7.3242]], device='cuda:0', dtype=torch.float16)

# scale = torch.tensor([[0.0029],
#         [0.0035],
#         [0.0038],
#         [0.0039],
#         [0.0056],
#         [0.0055]], device='cuda:0', dtype=torch.float16)


# zero[zero<0] = 0 #problem?
# z_max, s_max = zero.max().item(), scale.max().item()

# encode_nW_nbits = 16
# z_normalizer = (2**(encode_nW_nbits//2) - 1) / z_max # z_max = 2**nW_nbits - 1
# s_normalizer = (2**(encode_nW_nbits//2) - 1) / s_max

# # encode_nW_nbits = 32 
# # z_normalizer = 2048 
# # s_normalizer = 4096 

# # encode_nW_nbits = 16 #2x compression
# # z_normalizer = 16 #(2**(encode_nW_nbits//2) - 1) / z_max # z_max = 2**nW_nbits - 1
# # s_normalizer = 2048 #(2**(encode_nW_nbits//2) - 1) / s_max


# z_norm = (zero.float() *  z_normalizer).round().to(torch.int32)
# s_norm = (scale.float() * s_normalizer).round().to(torch.int32)

# zero_scale_quant = z_norm | (s_norm << (encode_nW_nbits//2))

# unpack_mask      = 2**(encode_nW_nbits//2) - 1

# # pre_shift        = 1 if (encode_nW_nbits==32) else 0 #to avoid inf with float16
# # zero_dequant     = (((zero_scale_quant.to(torch.int32) & unpack_mask) >> pre_shift).half()/z_normalizer) * (2**pre_shift)
# # scale_dequant    = ((((zero_scale_quant.to(torch.int32) >> (encode_nW_nbits//2))) >> pre_shift).half()/s_normalizer) * (2**pre_shift)

# zero_dequant     = (zero_scale_quant.to(torch.int32) & unpack_mask).float() / z_normalizer
# scale_dequant    = ((zero_scale_quant.to(torch.int32) >> (encode_nW_nbits//2))).float() / s_normalizer


# print('zero error',  100*(zero - zero_dequant).abs().mean()/zero.max()) #0.0138
# print('scale error', 100*(scale - scale_dequant).abs().mean()/scale.max()) #0.0001
...
class GemliteLinear(torch.nn.Module):
    def __init__(
        ...
        self.SUPPORTED_BITS = [5] #+ [3]

        super().__init__()
        if W_nbits not in self.SUPPORTED_BITS:
            raise NotImplementedError("Only 5 W_nbits are supported.")
    ...
    def pack(self, W_q, scales, zeros):
        ...
        self.W_q_2bit = self.pack_base((W_q.int() >> 1) & 0b1111, self.W_nbits_2)
...
def check_valid(x, W, quant_linear):
    ...
    try:
        assert (y_ref - y_q).abs().mean() < 5e-3
    except:
